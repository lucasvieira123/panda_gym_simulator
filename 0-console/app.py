...
from system_launcher import start_system

_CONFIGS_DIR    = Path(__file__).parent.parent / "1-manager" / "configs"
_MANAGER_WS_URL = "ws://localhost:8001/ws/state"

_SAMPLES = {
    "ARM Robot":         _CONFIGS_DIR / "arm"                / "asm.json",
    "Last Mile Delivery": _CONFIGS_DIR / "last_mile_delivery" / "asm.json",
}
...
def build_asm() -> dict:
    return {
        ...
        "nodes": {
            "__init__": {"name": "INIT", "type": "init"},
            "__end__":  {"name": "END",  "type": "end"},
        },
...
def _on_start_system():
    start_system()
    st.toast("Managing e Manager iniciados.")


def _on_load_sample():
    name = st.session_state._sample_select
    path = _SAMPLES[name]
    if path.exists():
        load_asm(json.loads(path.read_text(encoding="utf-8")))
        st.session_state._load_msg = f"Loaded: {name}"
    else:
        st.session_state._load_msg = f"❌ File not found: {path}"


def _on_file_upload():
    uploaded = st.session_state._file_upload
    if uploaded is None:
        return
    if uploaded.name == st.session_state.last_loaded_file:
        return
    try:
        model = json.load(uploaded)
        load_asm(model)
        st.session_state.last_loaded_file = uploaded.name
        st.session_state._load_msg = "Model loaded!"
    except Exception as e:
        st.session_state._load_msg = f"❌ Invalid file: {e}"
...
with st.sidebar:
    ...
    # ── Step 1: Load Model ────────────────────────────────────────────────────
    st.subheader("1 — Load ASM")
    st.caption("Upload a JSON file or pick a built-in sample.")
    st.file_uploader("Upload .json", type="json", key="_file_upload",
                     on_change=_on_file_upload, label_visibility="collapsed")

    st.caption("Or load a sample:")
    st.selectbox("Sample", list(_SAMPLES.keys()), key="_sample_select",
                 label_visibility="collapsed")
    st.button("Load Sample", use_container_width=True, on_click=_on_load_sample)

    if msg := st.session_state.pop("_load_msg", None):
        st.success(msg)

    st.divider()

    # ── Step 2: ASM Editor ────────────────────────────────────────────────────
    st.subheader("2 — ASM Editor")
    st.caption("Edit the model, then save before starting.")
    ...
    if st.button("Save ASM", use_container_width=True):
        (_CONFIGS_DIR / "arm").mkdir(exist_ok=True)
        (_CONFIGS_DIR / "arm" / "asm.json").write_text(
            json.dumps(build_asm(), indent=2), encoding="utf-8"
        )
        st.success("Saved → 1-manager/configs/arm/asm.json")
    ...
    # ── Step 3: Start System ──────────────────────────────────────────────────
    st.subheader("3 — Start System")
    st.caption("Launch managing and manager in separate terminals.")
    st.button("▶ Start System", use_container_width=True, type="primary",
              on_click=_on_start_system)
